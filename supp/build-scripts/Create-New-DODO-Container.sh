...
mkdir -p $NJ_HOME/working

## Import and data directory
export NJ_IMPORT=$NJ_HOME/working/dodo/neo4jImport
mkdir -p $NJ_IMPORT
export NJ_DATA=$NJ_HOME/working/dodo/neo4jData
...
mkdir -p $NJ_DATA
## Neo4j plugins: APOC
export NJ_PLUGINS=$NJ_HOME/working/dodo/neo4jPlugins
mkdir -p $NJ_PLUGINS
cd $NJ_PLUGINS
...
cd $NJ_ROOT/build-scripts/
Rscript compile-feed-DODO-instance.R $TYPE $NAME

## Create backup
mkdir $NJ_HOME/ARCHIVE/$DODO_VERSION
cp $NJ_HOME/data/$NAME-neo4j-input-files.rda $NJ_HOME/ARCHIVE/$DODO_VERSION/
