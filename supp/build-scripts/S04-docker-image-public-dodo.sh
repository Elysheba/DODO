...
echo "FROM neo4j:$NJ_VERSION" > $NJ_HOME/working/dodo/Dockerfile
echo "COPY neo4jPlugins /plugins" >> $NJ_HOME/working/dodo/Dockerfile
echo "COPY neo4jData/data /data" >> $NJ_HOME/working/dodo/Dockerfile

cd $NJ_HOME/working/dodo/
docker build -t elysheba/$NAME:$DODO_VERSION .

cd $NJ_ROOT/
mkdir -p DODO-images
docker save elysheba/$NAME:$DODO_VERSION > $NJ_HOME/DODO-images/docker-$NAME-$DODO_VERSION.tar
...
## Load latest image
docker image load -i $NJ_HOME/DODO-images/docker-$NAME-$DODO_VERSION.tar
