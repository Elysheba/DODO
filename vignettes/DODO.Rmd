...
The graph database has been implemented with Neo4j 3.4.9 [@Neo4j2020], the DODO R package depends on the following packages available:
...
The DODO R package defines new S3 object called *disNet* (or *setDisNet*) contained all information on the nodes and the relationships between them. This object is generated when querying the database using the R package and can be modified or extended with the function described below. In general, the *disNet* object consists of seven components: **nodes** containing general information about the nodes, **syn** containing all synonyms available for the nodes, **xref** listing any *is_xref* or *is_related* relations between the nodes, **child** listing any *is_a* between the nodes, **pheno** listing phenotype/disease relations that exist, **alt** listing which identifiers might be deprecated, and **seed** the identifiers on which the *disNet* was originally constructed. A *setDisNet* object is a list of *disNet*. 
...
DODO R package provides function to allow four different scopes: building and interacting with a disNet of setDisNet object, visualizing a disNet, converting disease and phenotype concepts to different ontologies, and several utility functions to connect to DODO graph database or obtain low level information on identifiers. The table belows briefly list all function available within the package, as well as a short description and identification of the scope. 
...
As a consequence of the different way ontologies define disease (or phenotype) concepts, some cross-reference edges connect identifiers that are not exactly equal and therefore, some cross-reference edges are trusted more than others. Some ontologies (e.g. MONDO, EFO) use more narrow definition than others (eg. ICD10, ICD9). If cross-reference edges are considered equal without taking this information into account, it might result in the return of more distantly related concepts (Figure [fig:example_amb]). 

```{r, include = TRUE}
disNet <- build_disNet(id = "ORPHA:192")
disNet <- extend_disNet(disNet)
explore_disNet(disNet)
``` 
Figure (fig:example_amb): The EFO (and Orphanet) identifier for Coffin-Lowry syndrome (ORPHA:192) is cross-referenced to 21 different identifiers. Many of these cross-references are more or less equivalent to the original identifier. However, among its cross-references it also links to an ICD10 identifier (Q87.0) which is a broad term of "Congenital malformation syndromes predominantly affecting facial appearance". This identifier is highly ambiguous and links directly to 284 disease concepts. 

Therefore, there are two types of cross-reference edges encoded in DODO: *is_xref* and *is_related*. The *is_xref* edge is used for equal cross-reference relationships where the concepts relate more directly to each other (similar concept levels). The *is_related* edge is used for all other cross-reference edges. These edges are defined based on the sum of forward and backward ambiguities between databases to quantify the symmetry between them (included ontologies for this quantification are: MoNDO, EFO, Orphanet, Orphanet, OMIM, MedGen, UMLS, MeSH, ICD10, ICD9, DOID, ClinVar, MEDDRA, GARD, SNOMEDct, andNCIt). Ontologies with an ambiguity equal or lower than four are considered as *is_xref* with the exception of ICD10 and ICD9 which are never an *is_xref* edge except between themselves. An additional exception is put in place for OMIM and Orphanet, which often defines very narrow subtypes of disease that are linked to a variant and therefore encoded as *is_xref* edge.
