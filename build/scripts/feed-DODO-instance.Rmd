...
# setwd("~/Documents/Development/neoDODO/build/working/")
##
...
connect_to_dodo(
   url = url,
   importPath = "/data/lfrancois/Development/neoDODO/build/working/neo4jImport"
...
toImport <- DODO_parentId %>%
  select("database"=DB, 
         ...
         "parentid"=parent,
         origin)
neoDODO:::load_parent_identifiers(toImport=toImport, concept=concept)
```
...
neoDODO:::set_labels(concept = "Phenotype", nodes = nodes)

nodes <- DODO_entryId %>% filter(DB != "HP") %>% pull(dbid)
neoDODO:::set_labels(concept = "Disease", nodes = nodes)
...
toImport <- bind_rows(DODO_parentId %>% select(DB,
  ...
  select(database = DB,
         shortID = id,
         origin)
neoDODO:::load_concept_names(toImport = toImport, concept = concept)
```
