...
Calculate the forward and backward ambiguity on cross-reference edges. It also ensures that the nodes are bi-directional (dbid1 <-> dbid2) to ensure proper ambiguity calculations.

```{r ambiguity}
DODO_crossId <- DODO:::calculate_ambiguity(DODO_crossId = DODO_crossId)
tmp <- DODO_crossId %>% filter(BA > 1)
by(tmp$BA, INDICES = tmp$DB1, FUN = summary)
by(tmp$FA, INDICES = tmp$DB1, FUN = summary)
```

## Generate Dgraph objects 

The different DODO tables are converted in the Dgraph format.
...
```{r toDgraph}
list2env(DODO:::generate_dgraph_objects(
  DODO_entryId = DODO_entryId,
  DODO_crossId = DODO_crossId,
  DODO_altId = DODO_altId,
  DODO_pheno2dis = DODO_pheno2dis,
  DODO_parentId = DODO_parentId,
  DODO_idNames = DODO_idNames,
  edge_type = here("build/data/is_xref_edges.xlsx"),
  check = T),
  envir = .GlobalEnv)
if(is.null(dgraph_related)) rm(dgraph_related)
```

## Generate UID

The identifiers from the different Dgraph tables are recoded based on their object_type.

```{r create_uid}
dgraph_xref <- select(dgraph_xref, - FA, - BA)
dgraph_related <- select(dgraph_related, -FA, -BA)
dgraph_parentId <- select(dgraph_parentId, -origin)
dgraph_dbEdge <- select(dgraph_dbEdge, -id)

convertNodes <- function(x, entryNodes){
  if(attributes(x)$object_type %in% c("uid")){
    x$subject <- entryNodes$subject[match(x$subject,entryNodes$object)]
    x$object <- entryNodes$subject[match(x$object,entryNodes$object)]
  }else{
    x$subject <- entryNodes$subject[match(x$subject,entryNodes$object)]
  }
  return(x)
...
toConvert <- setdiff(grep("dgraph_",ls(),value = T),c("dgraph_entryNodes","dgraph_info"))
for(i in toConvert){
  print(i)
  object <- convertNodes(x = get(i), entryNodes = dgraph_entryNodes)
  assign(i, object)
}
lapply(toConvert,function(x) head(get(x), n = 2))

```

## Convert to RDF

The recoded Dgraph tables are appended and converted to RDF format.

```{r conv_rdf}
##=========================@
## **Convert to rdf ----
toRdf <- grep("dgraph_",ls(),value = T)

DODO_InputDgraph <- sapply(toRdf,
                      function(f){
                        print(f)
                        x <- DODO:::df2rdf(x = get(f))
                        return(x)
                      },
                      USE.NAMES = TRUE,
                      simplify = FALSE)
lapply(DODO_InputDgraph,function(x) head(x, n = 3))
