...
```{r listDB, out.width="50%", out.height= "75%", fig.cap="Overview of the number of nodes present for each disease ontology in DODO.", include = TRUE, echo = FALSE}
list_database() %>%
  arrange(desc(count)) %>%
...
Similarly, the *disNet* is used to identify disease variants reported for amyotrophic lateral sclerosis in ClinVar. `rlength(unique(clinvar_disnet$entrez))` unique Entrez gene identifiers are reported for `r length(unique(clinvar_disnet$dbid))` different disease identifiers (Table \@ref(tab:clinvarDisNetind)). 
