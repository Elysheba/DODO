...
gh <- c("https://github.com/Elysheba/Monarch",
        ...
        "https://github.com/Elysheba/HPO",
        "https://github.com/Elysheba/ClinVar",
...
```{r, include = TRUE, echo = FALSE}
if(!rerun){
  load(here("DODO_publication/data/als_disNet_to_externalResources.rda"))
  load(here("DODO_publication/data/als_direct_chembl_clinvar_results.rda"))
}else{
  ...
  save(disNet,
       chembl_disnet,
       clinvar_disnet,
       file = here("DODO_publication/data/als_disNet_to_externalResources.rda"))
  ...
  save(chembl_als, clinvar_als, file = here("DODO_publication/data/als_direct_chembl_clinvar_results.rda"))
}
```
...
include_graphics(here("DODO_publication/fig/disNet_als_extension.png"))
```
...
edges1 <- toShow$xref %>%
    dplyr::select(to,
                  from, 
                  type) %>%
    dplyr::mutate(color = dplyr::case_when(type == "is_related_nba" ~ "#fdbb84",
                                           TRUE ~ "#7fcdbb"),
                  arrows = "to")
edges2 <- toShow$xref %>%
  dplyr::select(to = from,
  ...
  dplyr::mutate(color = dplyr::case_when(type == "is_related_nba"  ~ "#fdbb84",
                                         TRUE ~ "#7fcdbb"),
                arrows = "to")
edges3 <- toShow$children %>%
  select(from = child,
         to = parent) %>%
  mutate(color = "#31a354",
         arrows = "to",
         type = "is_a")
edges <- bind_rows(edges1,
                   edges2,
                   edges3) %>%
  dplyr::distinct()
...
include_graphics(here("DODO_publication/fig/disNet_als_chembl.png"))
```
...
include_graphics(here("DODO_publication/fig/trace_connections.png"))
```
